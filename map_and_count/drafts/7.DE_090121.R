...
# chose CPM filtering value to be 0.5 as returned most amount of significant 
...
x_annot_pos <- max(both_CCNE_vs_HRD$nat_logFC) + 
  (max(both_CCNE_vs_HRD$nat_logFC)/15)
y_annot_pos <- max(both_CCNE_vs_HRD$my_logFC) + 
  (max(both_CCNE_vs_HRD$my_logFC)/15)

# plot nature vs my DE on density scatter:
p <- ggplot(both_CCNE_vs_HRD, aes(x=nat_logFC, y=my_logFC, colour = sig))
p <- p + geom_point() 
p <- p + scale_color_manual(values = c("#1B9E77", "grey"))
...
p <- p + scale_colour_discrete(labels = c("sig.", "non-sig."))
p <- p + annotate(
  geom="text", 
  x=x_annot_pos, 
  y=x_annot_pos, 
