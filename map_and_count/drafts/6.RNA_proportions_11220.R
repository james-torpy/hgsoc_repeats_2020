...
# split all_counts:
gc_counts <- all_counts[!(rownames(all_counts) %in% repeat_symbols),]
repeat_counts <- all_counts[rownames(all_counts) %in% repeat_symbols,]

# load logs:
gc_log <- read.table(
  paste0(ribo_dir, "Log.final.out"),
  stringsAsFactors = F
)
...
##########################################################################
### 2. Fetch total repeat counts from htseq counts file ###
##########################################################################


##########################################################################
### 3. Fetch total ribo mapped reads from STAR log ###
##########################################################################
